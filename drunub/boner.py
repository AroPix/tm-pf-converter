...
TM2 = False # for skin converter, dont change :0)
...
SOCKETS = ["LightFProj", # for converter :0)
           "LightFL1", 
           "LightFR1", 
           "LightRL", 
           "LightRR", 
...
CAR_SKEL = ["Body", ["FLHub", ["FLGuard", "FLWheel"], # stuff seems to break if bone indices are different
                               "FLReactor",
                               "FLArmBot",
                               "FLArmTop",
                               "FLSusp"],
...
for p in sorted(parts, key=lambda d: d.lod_level): # prioritise lod1 for bones
    ...
    if part_mesh and not is_socket:
        ...
        if mat_name:
            if lod_level > 3: # nadeo importer seems to explode if you use more than 3 lods, it will be corrected in skinfix.py
